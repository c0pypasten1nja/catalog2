...
def gconnect():
    ...
    if stored_access_token is not None and gplus_id == stored_gplus_id:
        response = make_response(json.dumps('Currently user is already connected.'), 200)
        response.headers['Content-Type'] = 'application/json'
        return response
    ...
    output += '<img src="'
    output += login_session['picture']
    output += '"style = "width: 300px; border-radius: 150px; -webkit-border-radius: 150px; -moz-border-radius: 150px;">'
...
def categoriesJSON(category_id):
    ...
    return jsonify(Wine=[w.serialize for w in wines])

@app.route('/catalog/JSON')
def catalogJSON():
    categorylist = session.query(Category).all()
    return jsonify(categorieslist = [c.serialize for c in category])
...
def editCategory(category_id):
  ...
  if request.method == 'POST':
    ...
    return render_template('editCategory.html', category_id = editedCategory.id, editedCategory = editedCategory)
...
def showWines(category_id):
    ...
    if 'username' not in login_session or creator.id != login_session['user_id']:
        return render_template('publicwines.html', wine = wine, category = category, creator = creator)
    else:
        return render_template('wines.html', wine = wine, category = category, creator = creator)
...
def newWine(category_id):
  ...
  creator = getUserInfo(category.user_id)
  if request.method == 'POST':
      newWine = Wine(name = request.form['name'], origin = request.form['origin'], description = request.form['description'], price = request.form['price'], category_id = category_id, user_id = creator.id)
      session.add(newWine)
      session.commit()
      flash('New Wine %s Item Successfully Created' % (newWine.name))
      return redirect(url_for('showWines', category_id = category_id))
  else:
      return render_template('newwineitem.html', category_id = category_id, creator = creator)
...
def editWineItem(category_id, wine_id):
    ...
    if request.method == 'POST':
        ...
        return render_template('editwineitem.html', category_id = category_id, wine_id = wine_id, editedWine = editedWine)
...
def deleteWineItem(category_id, wine_id):
    ...
    if request.method == 'POST':
        ...
        return render_template('deleteWineItem.html', category_id = category_id, wine_id = wine_id, wineToDelete = wineToDelete)
